...
export default function UserNav({ currentUser, setCurrentUser }) {
  ...
  return (
    <div className="ml-auto flex items-center space-x-7">
      <button onClick={() => {nagivate("/search/movie")}} className="truncate h-8 px-3 rounded-md shadow text-white bg-blue-500">寫影評</button>
      <button onClick={() => {openButton()}} className="relative flex items-center">
        ...
        <div className={`${isOpen ? "-bottom-[158px] md:-bottom-[170px] opacity-100" : "bottom-[170px] opacity-0" } absolute w-full flex flex-col ease-linear duration-300 bg-blue-500 text-white justify-center`}>
