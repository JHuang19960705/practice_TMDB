...
export default function RegisterComponent({ currentUser, setCurrentUser }) {
  ...
  return (
    <div class="absolute left-1/2 top-1/2 z-10 w-3/5 min-w-52 -translate-x-1/2 -translate-y-1/2 rounded-lg border border-blue-400 bg-blue-100 p-5 shadow-xl">
      <div onClick={() => { navigate("/firstEnroll") }} class="absolute right-1 top-1 h-5 w-5 cursor-pointer bg-white">
      ...
      <div class="mb-5 h-full">
        <label for="password"><div class="mb-2 ml-2 text-sm">密碼</div></label>
        <input
      ...
      <div class="mb-5 h-full">
        <label for="password"><div class="mb-2 ml-2 text-sm">身份</div></label>
        <input
          onChange={handleRole}
          type="text"
          class="h-9 w-full cursor-text rounded-2xl bg-gray-50 pl-4 text-xs outline-none hover:border hover:border-blue-500"
      ...
      <div class="mt-6 grid w-full md:grid-cols-2">
        ...
        <div class="mt-3 flex justify-center md:mt-0 md:justify-end">
          <button onClick={handleRegister} class="rounded border border-blue-300 bg-blue-500 px-2 py-1 text-sm font-bold text-white shadow-sm md:px-5 md:py-1 md:text-base">註冊會員</button>
        </div>
      </div>
    </div>
  );
